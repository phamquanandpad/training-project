...
func Test_PostTodo(t *testing.T) {
	type fields struct {
		mockTodoCreator *mock_usecase.MockTodoCreator
	}
	...
	type testcase struct {
		...
		args     args
		expected *todo_v1.PostTodoResponse
		wantErr  bool
	}
	...
	testTables := map[string]testcase{
		"Post Todo successfully": {
			prepare: func(f *fields) {
				f.mockTodoCreator.
					EXPECT().
					Create(gomock.Any(), &input.TodoCreator{
						Task:        "todo task 1",
						Description: cast.Ptr("todo description 1"),
						Status:      todo.InProcess,
					}).
					Return((*output.TodoCreator)(&todo.Todo{
						ID:          todo.TodoID(1),
						Task:        "todo task 1",
						Description: cast.Ptr("todo description 1"),
						Status:      todo.InProcess,
	...
	for name, tt := range testTables {
		t.Run(name, func(t *testing.T) {
			...
			mockTodoCreator := mock_usecase.NewMockTodoCreator(ctrl)

			tt.prepare(&fields{
				mockTodoCreator: mockTodoCreator,
			})
			...
			got, err := svc.PostTodo(tt.args.ctx, tt.args.req)
			if (err != nil) != tt.wantErr {
				t.Errorf("PostTodo() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			...
			if diff := cmp.Diff(tt.expected, got,
				...
				t.Errorf("PostTodo() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
