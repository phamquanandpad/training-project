...
func (db *TemplateDB) createDB() error {
	...
	if _, err := db.db.ExecContext(context.TODO(), createDBStmt); err != nil {
		return fmt.Errorf("create db: %w", err)
	}
...
func (db *TemplateDB) applySchema() error {
	...
	if _, err := db.db.ExecContext(context.TODO(), string(schemaStmt)); err != nil {
		return fmt.Errorf("apply schema: %w", err)
	}
...
func (db *TemplateDB) getForeignKeysFromTable(tableName string) ([]ForeignKey, error) {
	query := `
        ...
        WHERE
            ...
            AND tc.table_name = ?
    `
	rows, err := db.db.QueryContext(context.TODO(), query, db.dbName, tableName)
	if err != nil {
	...
	for rows.Next() {
		fk := ForeignKey{TableName: tableName}
		if err := rows.Scan(
			&fk.ConstraintName,
			&fk.ColumnName,
...
func (db *TemplateDB) NewTestDB() (*TestDB, error) {
	...
	for _, tableName := range db.tableNames {
		sb.WriteString(fmt.Sprintf(
			...
			dbName, tableName, db.dbName, tableName,
		))
		sb.WriteString(fmt.Sprintf(
	...
	for _, fk := range db.foreignKeys {
		sb.WriteString(fmt.Sprintf(
			...
			dbName, fk.TableName, fk.ConstraintName, fk.ColumnName,
			dbName, fk.ReferencedTableName, fk.ReferencedColumnName,
		))
	...
	if _, err := db.db.ExecContext(context.TODO(), sb.String()); err != nil {
